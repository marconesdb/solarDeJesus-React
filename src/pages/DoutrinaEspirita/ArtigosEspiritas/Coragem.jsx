import React from 'react';
import Navbar from '../../../Navbar';
import Footer from '../../../Footer';

function Coragem() {
  return (
    <div>
      <Navbar />
      
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold text-center mb-8 mt-20">A coragem da fé segundo a doutrina espírita</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="md:order-last">
            <img src="https://via.placeholder.com/300" alt="Imagem do Artigo" className="w-full rounded-lg" />
          </div>
          
          <div className='bg-black100 text-white rounded-lg '>
            {/* Artigo em Português */}
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4 mt-4">Introdução</p>
            <p className="text-lg mb-4 text-justify ml-4 mr-4">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer nec odio. Praesent libero. Sed cursus ante dapibus diam. Sed nisi. Nulla quis sem at nibh elementum imperdiet. Duis sagittis ipsum.
            </p>
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Benevolência: Amar o Próximo como a Si Mesmo</p>
            <p className="text-lg mb-4 text-justify ml-4 mr-4">
              Phasellus blandit leo ut odio. Maecenas ullamcorper, dui et placerat feugiat, eros pede varius nisi, condimentum viverra felis nunc et lorem. Sed magna purus, fermentum eu, tincidunt eu, varius ut, felis.
            </p>
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Indulgência: Compreensão e Aceitação das Fraquezas Humanas</p>
            <p className="text-lg text-justify ml-4 mr-4">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer nec odio. Praesent libero. Sed cursus ante dapibus diam. Sed nisi. Nulla quis sem at nibh elementum imperdiet. Duis sagittis ipsum.
            </p>

            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Perdão das Ofensas: A Liberação do Peso do Rancor</p>
            <p className="text-lg mb-4 text-justify ml-4 mr-4">
              Phasellus blandit leo ut odio. Maecenas ullamcorper, dui et placerat feugiat, eros pede varius nisi, condimentum viverra felis nunc et lorem. Sed magna purus, fermentum eu, tincidunt eu, varius ut, felis.
            </p>
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Conclusão: A Caridade como Fundamento da Vida Cristã</p>
            <p className="text-lg text-justify ml-4 mr-4">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer nec odio. Praesent libero. Sed cursus ante dapibus diam. Sed nisi. Nulla quis sem at nibh elementum imperdiet. Duis sagittis ipsum.
            </p>
            <p className="text-sm text-right mt-4 ml-4 mr-4">Data de Publicação: <strong>[01/01/2024]</strong><br />Autor: <strong>Marcone Silva de Brito</strong></p>

            {/* Artigo em Inglês */}
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Introdução</p>
            <p className="text-lg mb-4 text-justify ml-4 mr-4">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer nec odio. Praesent libero. Sed cursus ante dapibus diam. Sed nisi. Nulla quis sem at nibh elementum imperdiet. Duis sagittis ipsum.
            </p>
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Benevolência: Amar o Próximo como a Si Mesmo</p>
            <p className="text-lg mb-4 text-justify ml-4 mr-4">
              Phasellus blandit leo ut odio. Maecenas ullamcorper, dui et placerat feugiat, eros pede varius nisi, condimentum viverra felis nunc et lorem. Sed magna purus, fermentum eu, tincidunt eu, varius ut, felis.
            </p>
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Indulgência: Compreensão e Aceitação das Fraquezas Humanas</p>
            <p className="text-lg text-justify ml-4 mr-4">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer nec odio. Praesent libero. Sed cursus ante dapibus diam. Sed nisi. Nulla quis sem at nibh elementum imperdiet. Duis sagittis ipsum.
            </p>

            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Perdão das Ofensas: A Liberação do Peso do Rancor</p>
            <p className="text-lg mb-4 text-justify ml-4 mr-4">
              Phasellus blandit leo ut odio. Maecenas ullamcorper, dui et placerat feugiat, eros pede varius nisi, condimentum viverra felis nunc et lorem. Sed magna purus, fermentum eu, tincidunt eu, varius ut, felis.
            </p>
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Conclusão: A Caridade como Fundamento da Vida Cristã</p>
            <p className="text-lg text-justify ml-4 mr-4">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer nec odio. Praesent libero. Sed cursus ante dapibus diam. Sed nisi. Nulla quis sem at nibh elementum imperdiet. Duis sagittis ipsum.
            </p>
            <p className="text-sm text-right mt-4 ml-4 mr-4">Data de Publicação: <strong>[01/01/2024]</strong><br />Autor: <strong>Marcone Silva de Brito</strong></p>
            {/*  Fim - Artigo em Inglês*/}


            {/* ============= */}

            
            {/* Artigo em Espanhol */}

            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Introdução</p>
            <p className="text-lg mb-4 text-justify ml-4 mr-4">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer nec odio. Praesent libero. Sed cursus ante dapibus diam. Sed nisi. Nulla quis sem at nibh elementum imperdiet. Duis sagittis ipsum.
            </p>
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Benevolência: Amar o Próximo como a Si Mesmo</p>
            <p className="text-lg mb-4 text-justify ml-4 mr-4">
              Phasellus blandit leo ut odio. Maecenas ullamcorper, dui et placerat feugiat, eros pede varius nisi, condimentum viverra felis nunc et lorem. Sed magna purus, fermentum eu, tincidunt eu, varius ut, felis.
            </p>
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Indulgência: Compreensão e Aceitação das Fraquezas Humanas</p>
            <p className="text-lg text-justify ml-4 mr-4">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer nec odio. Praesent libero. Sed cursus ante dapibus diam. Sed nisi. Nulla quis sem at nibh elementum imperdiet. Duis sagittis ipsum.
            </p>

            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Perdão das Ofensas: A Liberação do Peso do Rancor</p>
            <p className="text-lg mb-4 text-justify ml-4 mr-4">
              Phasellus blandit leo ut odio. Maecenas ullamcorper, dui et placerat feugiat, eros pede varius nisi, condimentum viverra felis nunc et lorem. Sed magna purus, fermentum eu, tincidunt eu, varius ut, felis.
            </p>
            <p className="text-lg mb-4 text-justify font-bold ml-4 mr-4">Conclusão: A Caridade como Fundamento da Vida Cristã</p>
            <p className="text-lg text-justify ml-4 mr-4">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer nec odio. Praesent libero. Sed cursus ante dapibus diam. Sed nisi. Nulla quis sem at nibh elementum imperdiet. Duis sagittis ipsum.
            </p>
            <p className="text-sm text-right mt-4 ml-4 mr-4 mb-4">Data de Publicação: <strong>[01/01/2024]</strong><br />Autor: <strong>Marcone Silva de Brito</strong></p>

            {/* Fim - Artigo em Espanhol */}



          </div>

          
        </div>
        <div className="mt-8">
          <h2 className="text-2xl font-bold mb-4">Referências Bibliográficas</h2>
          <div className="bg-black100 p-4 rounded-lg text-white text-justify">
            <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit.  </p>
            <p>Voluptatum eius ex vitae reprehenderit minus!</p>
            <p>Possimus dolores, quasi dignissimos excepturi animi adipisci unde architecto, quo ea cupiditate ducimus autem, totam voluptatibus?</p>
          </div>
        </div>
      </div>
      
     

     
      <Footer />
    </div>
  );
}

export default Coragem;
